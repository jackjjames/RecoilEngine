...
class SDLGLRenderContext final : public IRenderContext
{
public:
	SDL_Window* CreateWindow(const CGlobalRendering& rendering, const char* title) const override
	{
		...
		const std::array aaLvls = {rendering.msaaLevel, rendering.msaaLevel / 2, rendering.msaaLevel / 4, rendering.msaaLevel / 8, rendering.msaaLevel / 16, rendering.msaaLevel / 32, 0};
		...
		rendering.UpdateWindowBorders(newWindow);
		return newWindow;
	}
	...
	SDL_GLContext CreateContext(const CGlobalRendering& rendering, SDL_Window* window, const int2& minCtx) const override
	{
		...
		if ((newContext = SDL_GL_CreateContext(window)) != nullptr)
			return newContext;
		...
		char buf[1024] = {0};
		SNPRINTF(buf, sizeof(buf), frmts[false], __func__, SDL_GetError(), minCtx.x, minCtx.y, profs[rendering.forceCoreContext]);
	...
	void DestroyWindowAndContext(CGlobalRendering& rendering) const override
	{
		...
		SDL_GL_MakeCurrent(rendering.sdlWindow, nullptr);
...
std::unique_ptr<IRenderContext> CreateSDLGLRenderContext()
{
	return std::make_unique<SDLGLRenderContext>();
}
